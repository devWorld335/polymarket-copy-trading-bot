...
const validateNumericConfig = (): void => {
    ...
    // Validate trade monitor settings
    const positionUpdateInterval = parseInt(process.env.POSITION_UPDATE_INTERVAL_MS || '30000', 10);
    if (isNaN(positionUpdateInterval) || positionUpdateInterval < 1000) {
        throw new Error(
            `Invalid POSITION_UPDATE_INTERVAL_MS: ${process.env.POSITION_UPDATE_INTERVAL_MS}. Must be at least 1000ms.`
        );
    }

    const maxReconnectAttempts = parseInt(process.env.MAX_RECONNECT_ATTEMPTS || '10', 10);
    if (isNaN(maxReconnectAttempts) || maxReconnectAttempts < 1 || maxReconnectAttempts > 100) {
        throw new Error(
            `Invalid MAX_RECONNECT_ATTEMPTS: ${process.env.MAX_RECONNECT_ATTEMPTS}. Must be between 1 and 100.`
        );
    }

    const reconnectDelay = parseInt(process.env.RECONNECT_DELAY_MS || '5000', 10);
    if (isNaN(reconnectDelay) || reconnectDelay < 100) {
        throw new Error(
            `Invalid RECONNECT_DELAY_MS: ${process.env.RECONNECT_DELAY_MS}. Must be at least 100ms.`
        );
    }

    // Validate trade executor settings
    const tradeAggregationMinTotal = parseFloat(process.env.TRADE_AGGREGATION_MIN_TOTAL_USD || '1.0');
    if (isNaN(tradeAggregationMinTotal) || tradeAggregationMinTotal < 0.1) {
        throw new Error(
            `Invalid TRADE_AGGREGATION_MIN_TOTAL_USD: ${process.env.TRADE_AGGREGATION_MIN_TOTAL_USD}. Must be at least 0.1.`
        );
    }

    const executorLoopDelay = parseInt(process.env.EXECUTOR_LOOP_DELAY_MS || '300', 10);
    if (isNaN(executorLoopDelay) || executorLoopDelay < 50) {
        throw new Error(
            `Invalid EXECUTOR_LOOP_DELAY_MS: ${process.env.EXECUTOR_LOOP_DELAY_MS}. Must be at least 50ms.`
        );
    }

    const waitingMessageInterval = parseInt(process.env.WAITING_MESSAGE_INTERVAL_MS || '300', 10);
    if (isNaN(waitingMessageInterval) || waitingMessageInterval < 100) {
        throw new Error(
            `Invalid WAITING_MESSAGE_INTERVAL_MS: ${process.env.WAITING_MESSAGE_INTERVAL_MS}. Must be at least 100ms.`
        );
    }

    // Validate post order settings
    const minOrderSizeUSD = parseFloat(process.env.MIN_ORDER_SIZE_USD || '1.0');
    if (isNaN(minOrderSizeUSD) || minOrderSizeUSD < 0.1) {
        throw new Error(
            `Invalid MIN_ORDER_SIZE_USD: ${process.env.MIN_ORDER_SIZE_USD}. Must be at least 0.1.`
        );
    }

    const minOrderSizeTokens = parseFloat(process.env.MIN_ORDER_SIZE_TOKENS || '1.0');
    if (isNaN(minOrderSizeTokens) || minOrderSizeTokens < 0.1) {
        throw new Error(
            `Invalid MIN_ORDER_SIZE_TOKENS: ${process.env.MIN_ORDER_SIZE_TOKENS}. Must be at least 0.1.`
        );
    }

    const balanceSafetyBuffer = parseFloat(process.env.BALANCE_SAFETY_BUFFER || '0.99');
    if (isNaN(balanceSafetyBuffer) || balanceSafetyBuffer <= 0 || balanceSafetyBuffer > 1) {
        throw new Error(
            `Invalid BALANCE_SAFETY_BUFFER: ${process.env.BALANCE_SAFETY_BUFFER}. Must be between 0 and 1 (e.g., 0.99 = 99%).`
        );
    }

    // Validate network/retry settings
    const networkRetryDelay = parseInt(process.env.NETWORK_RETRY_DELAY_MS || '1000', 10);
    if (isNaN(networkRetryDelay) || networkRetryDelay < 100) {
        throw new Error(
            `Invalid NETWORK_RETRY_DELAY_MS: ${process.env.NETWORK_RETRY_DELAY_MS}. Must be at least 100ms.`
        );
    }

    // Validate display settings
    const topPositionsUserCount = parseInt(process.env.TOP_POSITIONS_USER_COUNT || '5', 10);
    if (isNaN(topPositionsUserCount) || topPositionsUserCount < 1 || topPositionsUserCount > 50) {
        throw new Error(
            `Invalid TOP_POSITIONS_USER_COUNT: ${process.env.TOP_POSITIONS_USER_COUNT}. Must be between 1 and 50.`
        );
    }

    const topPositionsTraderCount = parseInt(process.env.TOP_POSITIONS_TRADER_COUNT || '3', 10);
    if (isNaN(topPositionsTraderCount) || topPositionsTraderCount < 1 || topPositionsTraderCount > 50) {
        throw new Error(
            `Invalid TOP_POSITIONS_TRADER_COUNT: ${process.env.TOP_POSITIONS_TRADER_COUNT}. Must be between 1 and 50.`
        );
    }
};
...
const validateUrls = (): void => {
    ...
    if (process.env.RTDS_URL && !process.env.RTDS_URL.startsWith('ws')) {
        console.error('\n❌ Invalid RTDS_URL\n');
        console.error(`Current value: ${process.env.RTDS_URL}`);
        console.error('Default value: wss://ws-live-data.polymarket.com\n');
        console.error('⚠️  Use the default value unless you have a specific reason to change it!\n');
        throw new Error(
            `Invalid RTDS_URL: ${process.env.RTDS_URL}. Must be a valid WebSocket URL (ws:// or wss://).`
        );
    }
};
...
export const ENV = {
    ...
    TRADE_AGGREGATION_MIN_TOTAL_USD: parseFloat(process.env.TRADE_AGGREGATION_MIN_TOTAL_USD || '1.0'),
    // Trade monitor settings
    RTDS_URL: process.env.RTDS_URL || 'wss://ws-live-data.polymarket.com',
    POSITION_UPDATE_INTERVAL_MS: parseInt(process.env.POSITION_UPDATE_INTERVAL_MS || '30000', 10),
    ...
    EXECUTOR_LOOP_DELAY_MS: parseInt(process.env.EXECUTOR_LOOP_DELAY_MS || '300', 10),
    WAITING_MESSAGE_INTERVAL_MS: parseInt(process.env.WAITING_MESSAGE_INTERVAL_MS || '300', 10),
    // Post order settings
    ...
    BALANCE_SAFETY_BUFFER: parseFloat(process.env.BALANCE_SAFETY_BUFFER || '0.99'),
    // Network/retry settings
    NETWORK_RETRY_DELAY_MS: parseInt(process.env.NETWORK_RETRY_DELAY_MS || '1000', 10),
    // Display settings
    TOP_POSITIONS_USER_COUNT: parseInt(process.env.TOP_POSITIONS_USER_COUNT || '5', 10),
    TOP_POSITIONS_TRADER_COUNT: parseInt(process.env.TOP_POSITIONS_TRADER_COUNT || '3', 10),
